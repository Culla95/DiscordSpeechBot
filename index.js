    ...
    voice_Connection.on('speaking', async (user, speaking) => {
        ...
        audioStream.on('end', async () => {
            ...
            try {
                ...
                console.log("Pretranscribe")
                let out = await transcribe(new_buffer);
                console.log("Posttranscribe")
                if (out != null)
                    console.log("preprocesscommands")
                    process_commands_query(out, mapKey, user.id);
                    console.log("posprocesscommands")
            } catch (e) {
...
function process_commands_query(query, mapKey, userid) {
    ...
    var m = query.toLowerCase().match(regex);
    console.log("m: "+ m)
    if (m && m.length) {
        const cmd = (m[1]||'').trim();
        console.log("cmd: "+ cmd)
...
async function transcribe_witai(buffer) {
    ...
    try {
        ...
        var extractSpeechIntent = util.promisify(witClient.extractSpeechIntent);
        console.log("extractSpeechIntent type: "+typeof extractSpeechIntent)
        var stream = Readable.from(buffer);
        const contenttype = "audio/raw;encoding=signed-integer;bits=16;rate=48k;endian=little"
        var output = await extractSpeechIntent(WITAPIKEY, stream, contenttype)
        console.log("output type: "+typeof output)
        witAI_lastcallTS = Math.floor(new Date());
        var prueba = output.split(",");
        var pruebaaaa = prueba[prueba.length-2];
        console.log("pruebaaaa type: "+typeof pruebaaaa)
        console.log("transcribewitaioutput: " + output)
        //console.log("pruebaaaa: " + pruebaaaa)
        pruebaaaa = pruebaaaa.substr(12,(pruebaaaa.length-1));
        console.log("pruebaaaa: " + pruebaaaa)
        stream.destroy()
